...
The server SHALL expose an MCP tool named `get_controller` that takes a required integer `controller_id` and returns the matching controller's details, or an error if the controller does not exist or does not belong to the authenticated user.
