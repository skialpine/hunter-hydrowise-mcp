# irrigation-patch-tools Specification

## Purpose
Focused read-merge-write patch tools for incremental irrigation schedule edits. Each tool reads the current state, mutates one targeted field, and dispatches the full payload — the caller only supplies the field being changed. Created by archiving change add-patch-tools-for-incremental-edits.
## Requirements
