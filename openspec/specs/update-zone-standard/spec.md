# update-zone-standard Specification

## Purpose
Expose an MCP tool that wraps the upstream `updateZoneStandard` mutation for per-zone settings on STANDARD-mode controllers, keeping STANDARD-mode writes separate from the ADVANCED-mode `update_zone_settings` tool.
