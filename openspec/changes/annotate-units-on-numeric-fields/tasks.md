...
- [ ] 1.5 Probe `fixedWateringFrequency` and `virtualSolarSyncWateringFrequency` defaults — set known values via GUI and observe. (`smartWateringFrequency` default `86400` is already high-confidence seconds; no probe needed.) **→ DEFERRED** — read path found (`Controller.wateringPrograms[].fixedWateringProgram.period.{value,label}` via raw GraphQL probe 2026-05-10); `smartWateringProgram.period: {value:86400, label:"Once a day"}` confirms seconds for smart. But all 4 Heller Tufts WateringPrograms are in "Every Program Start Time" mode (fixed/vss value=60 is an inert default — label gives no unit info). Need a WateringProgram set to a specific fixed-interval (e.g. "every 3 days") to observe the actual unit. Whitelisted in `IDENTIFIER_WHITELIST` with DEFERRED comment.
