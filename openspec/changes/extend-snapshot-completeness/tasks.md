...
- [ ] 1.1 Extend `CONTROLLER_FIELDS` in `src/hydrawise/queries.ts` to include `location { coordinates { latitude, longitude }, address, country, state, locality }`, `settings { timeZone { id, name }, zones { interZoneDelay, masterZone { zoneNumber { value }, delay, postTimer } } }`, `masterZone { zoneNumber { value }, delay, postTimer }`, `expanders { id, name, number, hardware { model { id }, firmware { type, version } } }`, `runTimeGroups { id, name, duration }`, `controllerNotes { id, note, type, pinnedToTop, lastUpdatedAt { value } }`, `hardware.modules { id, name, serialNumber, moduleType, firmwareVersion }`
- [ ] 1.2 Extend `ZONE_FULL_QUERY` to include `masterValve`, `zoneNotes { id, note, type, pinnedToTop, lastUpdatedAt { value } }`
- [ ] 1.3 Extend `PROGRAM_START_TIMES_QUERY` to select `wateringDays`
- [ ] 1.4 Extend `PROGRAMS_FULL_QUERY` (or add a new sibling) to select `timeRange { validFrom, validTo }`, `conditionalWateringAdjustments(controllerId) { id, label }` on the StandardProgram fragment
- [ ] 1.5 Add TypeScript interfaces for the new types (`LocationRead`, `ExpanderRead`, `RunTimeGroupRead`, `ControllerNoteRead`, `ZoneNoteRead`, `ModuleRead`, `MasterValveRead`, `TimeZoneRead`)

## 2. Schema and types — add mutation strings

- [ ] 2.1 Add `UPDATE_LOCATION_MUTATION` and `UPDATE_LOCATION_COORDINATES_MUTATION`
- [ ] 2.2 Add `UPDATE_CONTROLLER_MASTER_VALVE_MUTATION`
- [ ] 2.3 Add `UPDATE_CONTROLLER_PROGRAM_MODE_MUTATION`
...
- [ ] 3.1 Update `Controller`, `Zone`, `ZoneRichRead` interfaces in `src/hydrawise/queries.ts` to reflect the extended read shapes
- [ ] 3.2 Update `getController`, `getZoneFull` in `src/hydrawise/api.ts` to surface the new fields (the queries change will populate them automatically; the typed result shape just needs updating)
- [ ] 3.3 Add `HydrawiseApi.updateLocation(deviceId, payload)` — accepts `{ address?, latitude?, longitude? }` and dispatches one or both upstream mutations; returns combined result
- [ ] 3.4 Add `HydrawiseApi.updateControllerMasterValve(controllerId, zoneNumber)`
- [ ] 3.5 Add `HydrawiseApi.updateControllerProgramMode(controllerId, mode)` — accepts `'STANDARD' | 'ADVANCED'`
...
- [ ] 5.1 Create `src/tools/controllerConfig.ts` with `registerControllerConfigTools(server, api, logger)` export
- [ ] 5.2 Implement `update_location` — accepts `controller_id`, optional `address`, optional `latitude`/`longitude`; at least one must be provided; preview shows the full payload of each underlying mutation
- [ ] 5.3 Implement `update_controller_master_valve` — `controller_id` + `zone_number`
...
- [ ] 6.1 Create `src/tools/notes.ts` with `registerNotesTools(server, api, logger)` export
- [ ] 6.2 Implement `list_controller_notes(controller_id)` — read-only
- [ ] 6.3 Implement `list_zone_notes(zone_id)` — read-only
- [ ] 6.4 Implement `create_controller_note`, `update_controller_note`, `delete_controller_note` with preview
- [ ] 6.5 Implement `create_zone_note`, `update_zone_note`, `delete_zone_note` with preview
- [ ] 6.6 Register the new tool group in `src/server.ts`'s `buildMcpServer`

## 7. Snapshot tool — extend `dump_controller_snapshot`

- [ ] 7.1 Update `ControllerSnapshotV1` (or rename to `ControllerSnapshotV2`) interface in `src/tools/backup.ts` to reflect the expanded shape; bump `SNAPSHOT_VERSION` to 2
- [ ] 7.2 Inside `dump_controller_snapshot`, after `getPrograms`, call `getStandardProgram(controller_id, program.id)` for each Standard program in parallel; replace each thin entry with the full detail
- [ ] 7.3 Surface `controller.location`, `controller.time_zone`, `controller.master_valve`, `controller.expanders`, `controller.modules`, `controller.controller_notes`, `controller.run_time_groups`, `controller.settings.zones.inter_zone_delay` in the envelope
- [ ] 7.4 Surface per-zone `master_valve_override` and `zone_notes` in each zone entry
- [ ] 7.5 Document the `_unreadable_fields` array in the snapshot envelope (per zone)
- [ ] 7.6 Update tool description to reflect the new envelope contents

## 8. Tests

- [ ] 8.1 Unit tests for `serializeLocation`, `serializeExpander`, `serializeNote`, `serializeRunTimeGroup`
...
- [ ] 9.1 Update CLAUDE.md MCP tools section: add the new `irrigation-controller-config` and `irrigation-notes` tools; document the expanded snapshot envelope
- [ ] 9.2 Update CLAUDE.md "Restore-from-backup" section with the new restorable categories
- [ ] 9.3 If `Zone.masterValve` turns out to be read-only via API (no `updateZoneAdvanced` arg for it), document the gap in the snapshot envelope and CLAUDE.md
