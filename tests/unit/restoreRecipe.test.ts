...
function makeMinimalSnapshot(overrides: Partial<SnapshotForRecipe['controller']> = {}): SnapshotForRecipe {
  return {
    ...
    controller: {
      ...
      location: null,
      master_valve: null,
      seasonal_adjustments: { factors: [100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100] },
    ...
    const recipe = buildRestoreRecipe(
      makeMinimalSnapshot({
        watering_triggers: {
          ...
          extend_water_temperature_percentage: 10,
          ...
          reduce_water_temperature_percentage: 30,
        },
      }),
    );
        ...
        programs: [
          {
            ...
            valid_from: null,
            valid_to: null,
            periodicity: { period: 1, series_start: null },
    ...
    const caveats = buildRestoreCaveats(
      makeMinimalSnapshot({
        watering_triggers: {
          ...
          reduce_water_temperature_percentage: 0,
        },
      }),
    );
