...
const CreateCustomSensorTypeInput = {
  ...
  delay_seconds: z.number().int().nullable().optional().describe('Activation delay before the sensor trips, in seconds (createCustomSensorType.delay Int).'),
  off_timer_seconds: z.number().int().nullable().optional().describe('Off-timer duration after sensor de-activates, in seconds (createCustomSensorType.offTimer Int).'),
...
const UpdateCustomSensorTypeInput = {
  ...
  custom_sensor_type: CustomSensorTypeEnumZ,
  mode_type: CustomSensorModeTypeEnumZ,
  delay_seconds: z.number().int().nullable().optional().describe('Activation delay before the sensor trips, in seconds (updateCustomSensorType.delay Int).'),
  off_timer_seconds: z.number().int().nullable().optional().describe('Off-timer duration after sensor de-activates, in seconds (updateCustomSensorType.offTimer Int).'),
  flow_sensor_rate: z.number().nullable().optional(),
  preview: z.boolean().optional(),
};
...
export function registerSensorTools(
  ...
  server.registerTool(
    ...
    {
      description:
        'List all sensors wired to a controller. Each entry has the writable shape (id, name, model_id, input_number, zone_ids) plus an _observed block with model details (model_name, sensor_type LEVEL_OPEN | LEVEL_CLOSED | FLOW | THRESHOLD, mode_type START | STOP | REPORT, divisor, flow_rate, off_level, off_timer_seconds, delay_seconds, active, input_label, type_label, category, customer_id). zone_ids is the array of zone IDs the sensor protects.',
  ...
  server.registerTool(
    ...
    {
      description:
        'List the available sensor model catalog — built-in Hydrawise types (rain/soil sensors, flow meters) plus any custom types created on this account. Each entry has id, name, sensor_type, mode_type, category, calibration fields (delay_seconds, off_timer_seconds, off_level, divisor, flow_rate), and customer_id (null for built-in, non-zero for customer-owned). Use this to map a snapshot\'s sensor model.name back to the current model.id at restore time. controller_id is currently informational; the catalog is account-wide.',
      ...
      wrap('create_custom_sensor_type', async () =>
        previewOrApply(
          'createCustomSensorType',
          {
            ...
            off_timer_seconds: off_timer_seconds ?? null,
