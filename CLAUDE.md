...
- **`updateZoneStandard` 500s when `sensorIds` is passed as `null`.** The schema declares `sensorIds: [Int] = []`, but the GraphQL default only applies when the variable is **omitted entirely** — explicitly passing `null` overrides the default and causes a server 500. **Fix:** the `updateZoneStandard` API wrapper conditionally includes `sensorIds` in the variables object only when `payload.sensor_ids != null`. **What `sensorIds` in `updateZoneStandard` actually controls is unknown.** The only live test (zone 2063156, 2026-05-10) was inconclusive because the account's only sensor (rain sensor id 132575) is attached to all 22 zones globally — a global sensor would survive `sensorIds: []` regardless. `list_zone_sensors` for that zone returned `zone_ids: []` after the call (possibly indicating the zone-level reference was cleared), while `list_sensors` still showed the sensor on all zones (sensor-side association intact). **Conservative guidance: always pass `sensor_ids` explicitly with the zone's current sensor IDs if you know them.** `sensor_ids` is in `_unreadable_fields` for most zones, so the recipe builder emits it as `null` and relies on the `notes` merge step.
