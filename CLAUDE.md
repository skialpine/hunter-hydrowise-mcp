...
- **`ZoneIcon.id` may be a stale reference to a deleted custom image.** The `ZoneIcon` type has `id: Int` (built-in icon template) and `customImage: File` (custom uploaded image). `ZONE_FULL_QUERY` only selects `icon { id }`. For zones that used a custom image, `ZoneIcon.id` can return the custom image's file ID (a large integer like 890232). If that custom image was subsequently deleted from Hydrawise, passing the stale ID in `$icon` will trigger an `internal` server error (not "Missing icon") and corrupt the zone's icon state to 0 — making ALL further mutations on that zone fail with the same internal error. **Recovery:** pass any valid built-in icon ID (e.g. `icon: 10`, used by several zones on this account) to restore the zone to a writable state. The icon can then be corrected through the Hydrawise app. Confirmed on 2026-05-10 (zone 2062869, `ZoneIcon.id = 890232`, custom image already deleted). **Do not assume `get_zone_settings` returns a currently-valid icon** — always prefer a built-in icon ID you can verify (e.g. from `get_zone_settings` on a zone that works) when the zone hasn't been mutated recently.
