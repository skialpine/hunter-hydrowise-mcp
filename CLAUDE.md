...
- **`Time` is a true scalar** (`scalar Time` in the live SDL), unlike `DateTime`. Do NOT add `{ value }` sub-selections to `Time` fields — that would cause a server-side GraphQL error. `ProgramStartTime.time: Time!` is selected as bare `time` in `PROGRAM_START_TIMES_QUERY` and deserializes as a plain string. `ProgramStartTimeRead.time` is typed `string | ...` to match.
