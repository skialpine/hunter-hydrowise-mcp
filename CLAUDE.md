...
- **ADVANCED-mode reads have NOT been validated against a real ADVANCED-mode controller.** The implementation is schema-correct (every field selected matches `schema/hydrawise.live.graphql`) and unit-tested with fixtures, but no live snapshot has been compared against the GUI for an ADVANCED account. The next user with such an account is the de-facto integration test.
...
Most accounts (including the dev account this project was built against) are Standard. ADVANCED-mode support is now read-complete and restore-complete — but **the read paths have NOT been validated against a real ADVANCED-mode controller** (the dev account, Heller Tufts, is STANDARD). The implementation is schema-correct and unit-tested with fixtures derived from `schema/hydrawise.live.graphql`. The next user with an ADVANCED-mode account is encouraged to run `dump_controller_snapshot` and verify the `controller.advanced_programs[]` and per-zone `settings.advanced_program` fields against what the GUI shows.
